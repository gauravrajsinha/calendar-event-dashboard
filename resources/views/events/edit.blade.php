                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="date" id="start_date" name="start_date"
                                value="{{ old('start_date', $event->start_time->format('Y-m-d')) }}" required
                                class="block w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary text-sm">
                            @error('start_date')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="date" id="end_date" name="end_date"
                                value="{{ old('end_date', $event->end_time->format('Y-m-d')) }}" required
                                class="block w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary text-sm">
                            @error('end_date')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="flex items-center">
                            <input type="checkbox" id="all_day" name="all_day" value="1"
                                {{ old('all_day', $event->all_day) ? 'checked' : '' }} class="mr-2">
                        ...
                        <div class="flex items-center">
                            <input type="checkbox" id="is_recurring" name="is_recurring" value="1"
                                {{ old('is_recurring', $event->is_recurring) ? 'checked' : '' }} class="mr-2">
                            <label for="is_recurring" class="text-sm text-gray-700">Recurring event</label>
                        </div>
                    </div>

                    <!-- Recurrence Pattern (shown/hidden based on current value) -->
                    <div id="recurrence_options" class="{{ old('is_recurring', $event->is_recurring) ? '' : 'hidden' }}">
                        ...
                        <div class="relative">
                            <select id="recurrence_pattern" name="recurrence_pattern"
                                class="block w-full pr-8 px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary text-sm appearance-none">
                                <option value="daily"
                                    {{ old('recurrence_pattern', $event->recurrence_pattern) == 'daily' ? 'selected' : '' }}>
                                    Daily</option>
                                <option value="weekly"
                                    {{ old('recurrence_pattern', $event->recurrence_pattern) == 'weekly' ? 'selected' : '' }}>
                                    Weekly</option>
                                <option value="biweekly"
                                    {{ old('recurrence_pattern', $event->recurrence_pattern) == 'biweekly' ? 'selected' : '' }}>
                                    Bi-weekly</option>
                                <option value="monthly"
                                    {{ old('recurrence_pattern', $event->recurrence_pattern) == 'monthly' ? 'selected' : '' }}>
                                    Monthly</option>
                            </select>
                            <div class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                                <div class="w-4 h-4 flex items-center justify-center text-gray-400">
                            ...
                            <select id="attendees" name="attendees[]" multiple
                                ...
                                @foreach ($users as $user)
                                    <option value="{{ $user->id }}"
                                        {{ in_array($user->id, old('attendees', $event->attendees->pluck('id')->toArray())) ? 'selected' : '' }}>
                ...
                <div class="mt-8 flex items-center justify-between">
                    <form action="{{ route('events.destroy', $event) }}" method="POST" class="inline"
                        ...
                        <button type="button" onclick="confirmDelete()"
                            class="px-4 py-2 border border-red-300 text-red-700 rounded-button text-sm font-medium hover:bg-red-50 focus:outline-none whitespace-nowrap">Delete
                            Event</button>
                    </form>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            allDayCheckbox.addEventListener('change', function() {
                if (this.checked) {
                    ...
                    startTimeInput.value = '{{ $event->start_time->format('H:i') }}';
                    endTimeInput.value = '{{ $event->end_time->format('H:i') }}';
                    startTimeInput.disabled = false;
                    endTimeInput.disabled = false;
                }
            });

            // Set initial state based on all_day value
